 ...
 *          Hengxu Li <li812080076@gmail.com> (Custom Cell Individual Offset (CIO) support)
 */
...
LteUeRrc::MeasurementReportTriggering(uint8_t measId)
{
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A3: {
             ...
             ++storedMeasIt)
        {
            ...
            // ★ lookup CIO for (servingCellId -> neighborCellId) and use it as Ocn
            ocn = CioStore::Get(servingCellId, cellId);
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A4: {
             ...
             ++storedMeasIt)
        {
            ...
            // ★ only evaluate cells on the frequency indicated by the associated measObjectEutra
            // ★ (no effect in single-frequency scenarios, avoids cross-frequency triggering in multi-frequency cases)
            if (m_storedMeasValues.at(cellId).carrierFreq != measObjectEutra.carrierFreq)
            {
                continue;
            }

            // ★ retrieve CIO for (servingCellId -> neighborCellId) and use it as Ocn
            ocn = CioStore::Get(servingCellId, cellId);
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A5: {
        ...
        if (entryCond)
        {
                 ...
                 ++storedMeasIt)
            {
                ...
                // ★ (no impact in single-frequency scenarios, prevents cross-frequency triggering in multi-frequency cases)
                if (m_storedMeasValues.at(cellId).carrierFreq != measObjectEutra.carrierFreq)
                {
                    continue;
                }

                // ★ set Ocn for this neighbor via CIO lookup
                ocn = CioStore::Get(servingCellId, cellId);
                ...
                else
                {
                         ...
                         ++storedMeasIt)
                    {
                        ...
                        if (measReportIt->second.cellsTriggeredList.find(cellId) !=
                            measReportIt->second.cellsTriggeredList.end())
                        {
                            // ★ filter cells by the frequency of the associated measObject
                            if (m_storedMeasValues.at(cellId).carrierFreq != measObjectEutra.carrierFreq)
                            {
                                continue;
                            }

                            // ★ assign Ocn for this neighbor using CIO lookup
                            ocn = CioStore::Get(servingCellId, cellId);
