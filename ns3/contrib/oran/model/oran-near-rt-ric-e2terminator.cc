    ...
    if (m_active)
    {
                 ...
                 TypeId::LookupByName("ns3::OranReportLteUeRsrpRsrq"))
        {
            ...
            m_data->SaveLteUeRsrpRsrq(rsrpRsrqRpt->GetReporterE2NodeId(),
                ...
                TypeId::LookupByName("ns3::OranReportLteEnbPrbUtilization"))
        {
            auto prb = report->GetObject<OranReportLteEnbPrbUtilization>();
            m_data->SaveLteEnbPrbUtilization(prb->GetReporterE2NodeId(),
                                            prb->GetCellId(),
                                            prb->GetDlUtilization(),
                                            prb->GetUlUtilization(),
                                            prb->GetWindowMs(),
                                            prb->GetTtisObserved(),
                                            prb->GetTime());
        }
        else if (report->GetInstanceTypeId() ==
                TypeId::LookupByName("ns3::OranReportLteUeSinr"))
        {
            auto r = report->GetObject<OranReportLteUeSinr>();
            m_data->SaveLteUeSinr(r->GetReporterE2NodeId(),
                                    r->GetCellId(),
                                    r->GetRnti(),
                                    r->GetSinrLinear(),
                                    r->GetSinrDb(),
                                    r->GetRsrpMw(),
                                    r->GetRsrpDbm(),
                                    r->GetTime());   // <-- use base OranReport time
        }
        else if (auto tp = DynamicCast<OranReportLteEnbScheduledThroughput>(report))
        {
            m_data->SaveLteEnbScheduledThroughput(
                tp->GetReporterE2NodeId(),
                tp->GetCellId(),
                tp->GetDlMbps(),
                tp->GetUlMbps(),
                tp->GetWindowMs(),
                ...
                TypeId::LookupByName ("ns3::OranReportLteUePdcpThroughput"))
        {
        auto tp = report->GetObject<OranReportLteUePdcpThroughput> ();
        m_data->SaveLteUePdcpThroughput (tp->GetUeId(),
                                        tp->GetCellId(),
                                        tp->GetDlMbps (),
                                        tp->GetUlMbps (),
                                        tp->GetTime ());
        }
        else if (auto mcs = DynamicCast<OranReportLteEnbMcs>(report))
        {
        m_data->SaveLteEnbMcs(mcs->GetReporterE2NodeId(),
                                mcs->GetCellId(),
                                mcs->GetDlMean(), mcs->GetDlP50(), mcs->GetDlP95(),
                                mcs->GetUlMean(), mcs->GetUlP50(), mcs->GetUlP95(),
                                mcs->GetTime());
        }
        else if (auto pr = DynamicCast<OranReportLteUePrb>(report))
        {
        m_data->SaveLteUePrb(pr->GetUeId(),
                            pr->GetCellId(),
                            pr->GetDlPrbs(),
                            pr->GetUlPrbs(),
                            pr->GetTime());
                ...
                TypeId::LookupByName("ns3::OranReportLteEnbUlInterf"))
        {
            auto ulinf = report->GetObject<OranReportLteEnbUlInterf>();
            m_data->SaveLteEnbUlInterference(
                ulinf->GetReporterE2NodeId(),
                ulinf->GetCellId(),
                ulinf->GetP95Mw(),
                ulinf->GetP95Dbm(),
                ulinf->GetTime());
        }
        else if (auto ev = DynamicCast<OranReportLteHoEvent>(report))
        {
        m_data->SaveLteHoEvent(ev->GetImsi(),
                                ev->GetUeId(),
                                ev->GetSrcCell(),
                                ev->GetDstCell(),
                                ev->GetEvent(),
        ...
        else if (auto dw = DynamicCast<OranReportLteUeDwell>(report))
        {
        m_data->SaveLteUeDwell(dw->GetUeId(),
                                dw->GetCellId(),
                                dw->GetDwellSeconds(),
                                dw->GetTime());
        }
        else if (auto uc = DynamicCast<OranReportLteEnbUeCount>(report))
        {
        m_data->SaveLteEnbUeCount(uc->GetReporterE2NodeId(),
                                    uc->GetCellId(),
                                    uc->GetUeCount(),
                                    uc->GetTime());
        }
