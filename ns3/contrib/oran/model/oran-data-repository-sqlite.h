...
class OranDataRepositorySqlite : public OranDataRepository
{
  public:
    ...
    void SaveLteEnbPrbUtilization (uint64_t e2NodeId,
                                   uint16_t cellId,
                                   double dlUtil,
                                   double ulUtil,
                                   uint32_t windowMs,
    ...
    GetAvgLteEnbPrbUtilization(uint64_t e2NodeId, Time tStart, Time tEnd) override;

    void SaveLteUeSinr(uint64_t e2NodeId, uint16_t cellId, uint16_t rnti,
                      double sinrLinear, double sinrDb, double rsrpMw, double rsrpDbm, Time t) override;

    std::tuple<bool,double,double>
    GetAvgLteUeSinr(uint64_t e2NodeId, Time tStart, Time tEnd) override;

    void SaveLteEnbScheduledThroughput(uint64_t e2NodeId,
                                       uint16_t cellId,
                                       double dlMbps,
                                       double ulMbps,
                                       uint32_t windowMs,
    ...
    void SaveLteUePdcpThroughput (uint64_t ueid,
                                  uint16_t cellId,
                                  double dlMbps,
                                  double ulMbps,
                                  Time t) override;

    void SaveLteEnbMcs(uint64_t e2nodeid,
                       uint16_t cellId,
                       double dlMean, double dlP50, double dlP95,
                       double ulMean, double ulP50, double ulP95,
                       Time t) override;

    void SaveLteUePrb(uint64_t ueid, uint16_t cellId,
                      uint64_t dlPrbs, uint64_t ulPrbs, Time t) override;
             
    void SaveLteEnbUlInterference(uint64_t e2nodeid,
                                  uint16_t cellId,
                                  double p95Mw,
                                  double p95Dbm,
                                  Time t) override;

    void SaveLteHoEvent(uint64_t imsi, uint64_t ueid,
                        uint16_t srcCell, uint16_t dstCell,
                        const std::string& event, Time t) override;

    void SaveLteUeDwell(uint64_t ueid, uint16_t cellId,
                        double dwellSeconds, Time t) override;
                     
    void SaveLteEnbUeCount(uint64_t e2nodeid,
                           uint16_t cellId,
                           uint32_t ueCount,
                           Time t) override;
  ...
  protected:
    ...
    enum StatementType
    {
        ...
        INSERT_LTE_ENB_PRB_UTIL,           //!< Insert eNB PRB utilization

        GET_AVG_LTE_ENB_PRB_UTIL,          //!< Average utilization query

        INSERT_LTE_ENB_SCHED_TP,

        INSERT_LTE_UE_PDCP_TP,

        INSERT_LTE_ENB_MCS,

        INSERT_LTE_UE_PRB,

        INSERT_LTE_ENB_UL_INTERF,

        INSERT_LTE_HO_EVENT,

        INSERT_LTE_UE_DWELL,

        INSERT_LTE_ENB_UECOUNT,
    ...
    enum CreateStatementType
    {
        ...
        TABLE_LTE_ENB_PRB_UTIL,           //!< Table with LTE eNB PRB utilization
        INDEX_LTE_ENB_PRB_UTIL_NODEID,    //!< Index on nodeid for PRB utilization

        TABLE_LTE_UE_SINR,                //!< Table with LTE UE SINR/RSRP
        INDEX_LTE_UE_SINR_NODEID,         //!< Index on nodeid for UE SINR/RSRP

        TABLE_LTE_ENB_SCHED_TP,
        INDEX_LTE_ENB_SCHED_TP_NODEID,

        TABLE_LTE_UE_PDCP_TP,
        INDEX_LTE_UE_PDCP_TP_UEID,

        TABLE_LTE_ENB_MCS,
        INDEX_LTE_ENB_MCS_CELL,
        INDEX_LTE_ENB_MCS_E2,

        TABLE_LTE_UE_PRB,
        INDEX_LTE_UE_PRB_UEID,
        
        TABLE_LTE_ENB_UL_INTERF,
        INDEX_LTE_ENB_UL_INTERF_E2_CELL,

        TABLE_LTE_HO_EVENTS,
        INDEX_LTE_HO_EVENTS_UEID,
        INDEX_LTE_HO_EVENTS_TIME,

        TABLE_LTE_UE_DWELL,
        INDEX_LTE_UE_DWELL_UEID,

        TABLE_LTE_ENB_UECOUNT,
        INDEX_LTE_ENB_UECOUNT_TIME,
        INDEX_LTE_ENB_UECOUNT_CELL,
