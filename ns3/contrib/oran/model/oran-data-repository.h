...
class OranDataRepository : public Object
{
  public:
    ...
    // Save eNB PRB utilization (values in [0.0, 1.0]).
    virtual void SaveLteEnbPrbUtilization (uint64_t e2NodeId,
                                          uint16_t cellId,
                                          double dlUtil,
                                          double ulUtil,
                                          uint32_t windowMs,
    ...
    // Convenience query for LMs (average utilization over window)
    virtual std::tuple<bool, double, double>
    GetAvgLteEnbPrbUtilization (uint64_t e2NodeId, Time tStart, Time tEnd) = 0;

    virtual void SaveLteUeSinr(uint64_t e2NodeId, uint16_t cellId, uint16_t rnti,
                              double sinrLinear, double sinrDb,
                              double rsrpMw, double rsrpDbm, Time t) = 0;

    virtual std::tuple<bool,double,double>
    GetAvgLteUeSinr(uint64_t e2NodeId, Time tStart, Time tEnd) = 0; // (avgSinrDb, avgRsrpDbm)

    // Save per-cell scheduled throughput (Mbps) and window metadata.
    virtual void SaveLteEnbScheduledThroughput(uint64_t e2NodeId,
                                              uint16_t cellId,
                                              double dlMbps,
                                              double ulMbps,
                                              uint32_t windowMs,
    ...
    // Save UE PDCP throughput (Mbps) over a window.
    virtual void SaveLteUePdcpThroughput (uint64_t ueid,
                                          uint16_t cellId,
                                          double dlMbps,
                                          double ulMbps,
                                          Time t) = 0;

    virtual void SaveLteEnbMcs(uint64_t e2nodeid,
                              uint16_t cellId,
                              double dlMean, double dlP50, double dlP95,
                              double ulMean, double ulP50, double ulP95,
                              Time t) = 0;

    virtual void SaveLteUePrb(uint64_t ueid, uint16_t cellId,
                              uint64_t dlPrbs, uint64_t ulPrbs, Time t) = 0;      

    virtual void SaveLteEnbUlInterference(uint64_t e2nodeid,
                                          uint16_t cellId,
                                          double p95Mw,
                                          double p95Dbm,
                                          Time t) = 0;

    virtual void SaveLteHoEvent(uint64_t imsi, uint64_t ueid,
                                uint16_t srcCell, uint16_t dstCell,
                                const std::string& event, Time t) = 0;

    virtual void SaveLteUeDwell(uint64_t ueid, uint16_t cellId,
                                double dwellSeconds, Time t) = 0;
                     
    virtual void SaveLteEnbUeCount(uint64_t e2nodeid,
                                  uint16_t cellId,
                                  uint32_t ueCount,
                                  Time t) = 0;
